...
raw_data <- read.csv("problems_project_euler.csv", stringsAsFactors = F)
...
for (row in 1:nrow(raw_data)) {
  ...
  nr_solutions <- as.numeric(strsplit(raw_data[row, ]$nr_solutions, split = ";")[[1]])
...
# Write into file
write.table(problems, 'sorted_problems_project_euler.csv', row.names = F)